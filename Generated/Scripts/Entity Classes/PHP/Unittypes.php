...
class Unittypes implements JsonSerializable {
     ...
     * @param $t1_object Unittypes
     * @return bool
     */
    public static function create($unittypes_object) {
        $conn = dbLogin();
        $sql = "INSERT INTO unittypes (Name) VALUES (\"" . $unittypes_object->Name . "\")";
     ...
     * @return bool
     */
    public static function update($unittypes_object) {
        $conn = dbLogin();
        $sql = "UPDATE unittypes SET Name = \"" . $unittypes_object->getName() . "\" WHERE ID = " . $unittypes_object->getID();
        $conn->query($sql);
    ...
    public static function delete($id) {
        $conn = dbLogin();
        $sql = "DELETE FROM unittypes WHERE ID = \"" . $id . "\"";
        $result = $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }
    ...
    public static function getSize() {
        $conn = dbLogin();
        $sql = "SHOW TABLE STATUS WHERE Name = \"unittypes\"";
        $result = $conn->query($sql);
    ...
    public static function searchByFields($fieldsAndValuesAssocArray) {
        ...
        $sql = "SELECT * FROM unittypes WHERE " . $combinedWhereClause;
        $result = $conn->query($sql);
     ...
     * @param $Unittypes_array Unittypes Array
     * @return bool|string
     */
    public static function toJSONArray($Unittypes_array) {
        $strArray = "[ ";
        foreach ($Unittypes_array as $i) {
