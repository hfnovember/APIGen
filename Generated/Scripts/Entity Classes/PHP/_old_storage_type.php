...
class _old_storage_type implements JsonSerializable {
     ...
     * @param $t1_object _old_storage_type
     * @return bool
     */
    public static function create($_old_storage_type_object) {
        $conn = dbLogin();
        $sql = "INSERT INTO _old_storage_type (type_name) VALUES (\"" . $_old_storage_type_object->type_name . "\")";
     ...
     * @return bool|_old_storage_type
     */
    public static function getByID($id) {
        $conn = dbLogin();
        $sql = "SELECT * FROM _old_storage_type WHERE id = " . $id;
        $result = $conn->query($sql);
        $sqlRowItemAsAssocArray = null;
        if ($result->num_rows > 0) {
            $sqlRowItemAsAssocArray = $result->fetch_assoc();
            $object = new _old_storage_type(
    ...
    public static function getMultiple($limit) {
        $conn = dbLogin();
        $sql = "SELECT * FROM _old_storage_type";
     ...
     * @return bool
     */
    public static function update($_old_storage_type_object) {
        $conn = dbLogin();
        $sql = "UPDATE _old_storage_type SET type_name = \"" . $_old_storage_type_object->getType_name() . "\" WHERE id = " . $_old_storage_type_object->getId();
        $conn->query($sql);
    ...
    public static function delete($id) {
        $conn = dbLogin();
        $sql = "DELETE FROM _old_storage_type WHERE id = \"" . $id . "\"";
        $result = $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }
    ...
    public static function getSize() {
        $conn = dbLogin();
        $sql = "SHOW TABLE STATUS WHERE Name = \"_old_storage_type\"";
        $result = $conn->query($sql);
    ...
    public static function searchByFields($fieldsAndValuesAssocArray) {
        ...
        $sql = "SELECT * FROM _old_storage_type WHERE " . $combinedWhereClause;
        $result = $conn->query($sql);
    ...
    public function jsonSerialize() {
        $jsonStr = "{\"id\": $this->id, \"type_name\": \"" . $this->type_name. "\" }";
        return $jsonStr;
    }
    
    /**
     * Converts an array of _old_storage_type objects to a JSON Array.
     * @param $_old_storage_type_array _old_storage_type Array
     * @return bool|string
     */
    public static function toJSONArray($_old_storage_type_array) {
        $strArray = "[ ";
        foreach ($_old_storage_type_array as $i) {
