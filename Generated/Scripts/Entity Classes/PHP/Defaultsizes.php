...
class Defaultsizes implements JsonSerializable {
     ...
     * @param $t1_object Defaultsizes
     * @return bool
     */
    public static function create($defaultsizes_object) {
        $conn = dbLogin();
        $sql = "INSERT INTO defaultsizes (Name, UnitTypeID) VALUES (\"" . $defaultsizes_object->Name . "\", $defaultsizes_object->UnitTypeID)";
     ...
     * @return bool|Defaultsizes
     */
    public static function getByID($id) {
        $conn = dbLogin();
        $sql = "SELECT * FROM defaultsizes WHERE ID = " . $id;
        $result = $conn->query($sql);
        $sqlRowItemAsAssocArray = null;
        if ($result->num_rows > 0) {
            $sqlRowItemAsAssocArray = $result->fetch_assoc();
            $object = new Defaultsizes(
     ...
     * @return bool
     */
    public static function update($defaultsizes_object) {
        $conn = dbLogin();
        $sql = "UPDATE defaultsizes SET Name = \"" . $defaultsizes_object->getName() . "\", UnitTypeID = " . $defaultsizes_object->getUnitTypeID() . " WHERE ID = " . $defaultsizes_object->getID();
        $conn->query($sql);
    ...
    public static function delete($id) {
        $conn = dbLogin();
        $sql = "DELETE FROM defaultsizes WHERE ID = \"" . $id . "\"";
        $result = $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }
    ...
    public static function getSize() {
        $conn = dbLogin();
        $sql = "SHOW TABLE STATUS WHERE Name = \"defaultsizes\"";
        $result = $conn->query($sql);
    ...
    public static function searchByFields($fieldsAndValuesAssocArray) {
        ...
        $sql = "SELECT * FROM defaultsizes WHERE " . $combinedWhereClause;
        $result = $conn->query($sql);
     ...
     * @param $Defaultsizes_array Defaultsizes Array
     * @return bool|string
     */
    public static function toJSONArray($Defaultsizes_array) {
        $strArray = "[ ";
        foreach ($Defaultsizes_array as $i) {
