...
class Producttypes implements JsonSerializable {
     ...
     * @param $t1_object Producttypes
     * @return bool
     */
    public static function create($producttypes_object) {
        $conn = dbLogin();
        $sql = "INSERT INTO producttypes (Name) VALUES (\"" . $producttypes_object->Name . "\")";
     ...
     * @return bool|Producttypes
     */
    public static function getByID($id) {
        $conn = dbLogin();
        $sql = "SELECT * FROM producttypes WHERE ID = " . $id;
        $result = $conn->query($sql);
        $sqlRowItemAsAssocArray = null;
        if ($result->num_rows > 0) {
            $sqlRowItemAsAssocArray = $result->fetch_assoc();
            $object = new Producttypes(
     ...
     * @return bool
     */
    public static function update($producttypes_object) {
        $conn = dbLogin();
        $sql = "UPDATE producttypes SET Name = \"" . $producttypes_object->getName() . "\" WHERE ID = " . $producttypes_object->getID();
        $conn->query($sql);
    ...
    public static function delete($id) {
        $conn = dbLogin();
        $sql = "DELETE FROM producttypes WHERE ID = \"" . $id . "\"";
        $result = $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }
    ...
    public static function getSize() {
        $conn = dbLogin();
        $sql = "SHOW TABLE STATUS WHERE Name = \"producttypes\"";
        $result = $conn->query($sql);
    ...
    public static function searchByFields($fieldsAndValuesAssocArray) {
        ...
        $sql = "SELECT * FROM producttypes WHERE " . $combinedWhereClause;
        $result = $conn->query($sql);
     ...
     * @param $Producttypes_array Producttypes Array
     * @return bool|string
     */
    public static function toJSONArray($Producttypes_array) {
        $strArray = "[ ";
        foreach ($Producttypes_array as $i) {
