<?php


/**********************************************************************************/
/*** THIS FILE HAS BEEN AUTOMATICALLY GENERATED BY THE PANICKAPPS API GENERATOR ***/

/*                It is HIGHLY suggested that you do not edit this file.          */

//  DATABASE:     easybusiness
//  FILE:         invoices.php
//  TABLE:        invoices
//  DATETIME:     2018-01-25 01:20:25pm
//  DESCRIPTION:  N/A

/**********************************************************************************/
			

class Invoices implements JsonSerializable {

	//-------------------- Attributes --------------------

    private $ID;
    private $CustomerID;
    private $InvoiceProductsID;
    private $Tax;

	//-------------------- Constructor --------------------

    public function __construct(
		$ID, 
		$CustomerID, 
		$InvoiceProductsID, 
		$Tax
		) {
        $this->ID = $ID;
		$this->CustomerID = $CustomerID;
		$this->InvoiceProductsID = $InvoiceProductsID;
		$this->Tax = $Tax;
    }

	//-------------------- Reflectors --------------------

	public static $allFields = array(
		"ID", 
		"CustomerID", 
		"InvoiceProductsID", 
		"Tax"
	);

	public static $hasUniqueFields = false;

	//-------------------- Getter Methods --------------------

	/**
     * @return int(11) unsigned
     */
     public function getID() { return $this->ID; }

	/**
     * @return int(10) unsigned
     */
     public function getCustomerID() { return $this->CustomerID; }

	/**
     * @return int(10) unsigned
     */
     public function getInvoiceProductsID() { return $this->InvoiceProductsID; }

	/**
     * @return double
     */
     public function getTax() { return $this->Tax; }


    /**
     * @return int(11) unsigned
     */
     public function getObjectID() { return $this->ID; }


	//-------------------- Setter Methods --------------------

	/**
     * @param $value int(10) unsigned
     */
     public function setCustomerID($value) { $this->CustomerID = $value; }

	/**
     * @param $value int(10) unsigned
     */
     public function setInvoiceProductsID($value) { $this->InvoiceProductsID = $value; }

	/**
     * @param $value double
     */
     public function setTax($value) { $this->Tax = $value; }


	//-------------------- Static (Database) Methods --------------------

            
            
    /**
     * Creates a database entry with the given object's data.
     * @param $t1_object Invoices
     * @return bool
     */
    public static function create($invoices_object) {
        $conn = dbLogin();
        $sql = "INSERT INTO invoices (CustomerID, InvoiceProductsID, Tax) VALUES ($invoices_object->CustomerID, $invoices_object->InvoiceProductsID, $invoices_object->Tax)";
        if ($conn->query($sql) === TRUE) return true;
        else return false;
    }


     /**
     * Retrieves a database entry matching the ID value provided.
     * @param $id int(11) unsigned
     * @return bool|Invoices
     */
    public static function getByID($id) {
        $conn = dbLogin();
        $sql = "SELECT * FROM invoices WHERE ID = " . $id;
        $result = $conn->query($sql);
        $sqlRowItemAsAssocArray = null;
        if ($result->num_rows > 0) {
            $sqlRowItemAsAssocArray = $result->fetch_assoc();
            $object = new Invoices(
				$sqlRowItemAsAssocArray["ID"], 
				$sqlRowItemAsAssocArray["CustomerID"], 
				$sqlRowItemAsAssocArray["InvoiceProductsID"], 
				$sqlRowItemAsAssocArray["Tax"]);
            return $object;
        }
        else return false;
    }


    /**
     * Retrieves all entries or up to a specified limit from the database. Use 0 or negative values as limit to retrieve all entries.
     * @param $limit int
     * @return array|bool
     */
    public static function getMultiple($limit) {
        $conn = dbLogin();
        $sql = "SELECT * FROM invoices";
        if ($limit > 0) $sql .= " LIMIT " . $limit;
        $result = $conn->query($sql);
        $itemsArray = array();
        if ($result->num_rows > 0) {
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    $object = new Invoices(
				$row["ID"], 
				$row["CustomerID"], 
				$row["InvoiceProductsID"], 
				$row["Tax"]);
                    array_push($itemsArray, $object);
                }
            }
            return $itemsArray;
        }
        return false;
    }


    /**
     * Updates a database entry with the given object's data.
     * @param $t1_object Invoices
     * @return bool
     */
    public static function update($invoices_object) {
        $conn = dbLogin();
        $sql = "UPDATE invoices SET CustomerID = " . $invoices_object->getCustomerID() . ", InvoiceProductsID = " . $invoices_object->getInvoiceProductsID() . ", Tax = " . $invoices_object->getTax() . " WHERE ID = " . $invoices_object->getID();
        $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }


    /**
     * Deletes an entry from the database given the object's data.
     * @param $id Int(11) unsigned
     * @return bool
     */
    public static function delete($id) {
        $conn = dbLogin();
        $sql = "DELETE FROM invoices WHERE ID = \"" . $id . "\"";
        $result = $conn->query($sql);
        if ($conn->affected_rows > 0) return true;
        else return false;
    }


    /**
     * Returns the number of entries in the database.
     * @return int
     */
    public static function getSize() {
        $conn = dbLogin();
        $sql = "SHOW TABLE STATUS WHERE Name = \"invoices\"";
        $result = $conn->query($sql);
        if ($result) return $result->fetch_object()->Rows;
        else return false;
    }


    /**
     * Returns true if the database is empty or false otherwise.
     * @return bool
     */
    public static function isEmpty() {
        $size = self::getSize();
        return ($size == 0);
    }


    /**
     * Searches the database for matches in the given field-value pairs in the given associative array.
     * The array should be in the format FieldName -> ValueToSearch where both values are strings.
     * @return array|bool
     */
    public static function searchByFields($fieldsAndValuesAssocArray) {
        $conn = dbLogin();
        
        $combinedWhereClause = "";
        
        foreach ($fieldsAndValuesAssocArray as $field => $value) {
            $combinedWhereClause .= $field . " = \"" . $value . "\" AND ";
        }
        $combinedWhereClause = substr($combinedWhereClause, 0, strlen($combinedWhereClause) - 4);
        
        $sql = "SELECT * FROM invoices WHERE " . $combinedWhereClause;
        $result = $conn->query($sql);
        if (!$result) return false;
        $itemsArray = array();
        while ($row = $result->fetch_assoc()) {
            $object = new Invoices(
				$row["ID"], 
				$row["CustomerID"], 
				$row["InvoiceProductsID"], 
				$row["Tax"]);
            array_push($itemsArray, $object);
        }
        return $itemsArray;
    }

	//-------------------- JSON Generation Methods --------------------

    /**
     * Specifies how objects of this class should be converted to JSON format.
     * @return string
     */
    public function jsonSerialize() {
        $jsonStr = "{\"ID\": $this->ID, \"CustomerID\": $this->CustomerID, \"InvoiceProductsID\": $this->InvoiceProductsID, \"Tax\": $this->Tax }";
        return $jsonStr;
    }
    
    /**
     * Converts an array of Invoices objects to a JSON Array.
     * @param $Invoices_array Invoices Array
     * @return bool|string
     */
    public static function toJSONArray($Invoices_array) {
        $strArray = "[ ";
        foreach ($Invoices_array as $i) {
            $strArray .= $i->jsonSerialize() . ", ";
        }
        $strArray = substr($strArray, 0, strlen($strArray) - 3);
        $strArray .= "} ] ";
        return $strArray;
    }

}

?>